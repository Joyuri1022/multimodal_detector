...
def render_prediction_card(
    ...
    predicted_color = LABEL_COLORS[predicted_name]
    true_color = LABEL_COLORS["True"]
    fake_color = LABEL_COLORS["Fake"]
    label_badge = (
        f"<span style='display:inline-block;padding:0.2rem 0.6rem;border-radius:999px;"
        f"background:{predicted_color};color:#ffffff;font-weight:700;'>{predicted_name}</span>"
    )
    probability_rows = "".join(
        f"""
        <div class="probability-row">
            <span style="color:{LABEL_COLORS[label_name]};font-weight:700;">{label_name}</span>
            <strong style="color:{LABEL_COLORS[label_name]};">{probabilities[label_name]:.4f}</strong>
        </div>
        """
    ...
    st.markdown(
        ...
        <div class="result-card">
            ...
            <strong>Meaning:</strong> {label_badge}<br/>
            <strong>Confidence:</strong> {confidence:.4f}<br/>
            <strong>Decision threshold (Fake):</strong> {threshold:.2f}<br/>
            <strong>Fake probability margin:</strong> <span style="color:{fake_color};">{margin:.4f}</span><br/>
            <strong>Prediction strength:</strong> {strength}<br/>
            <strong>Decision rule:</strong> <span style="color:{fake_color};">Fake probability {fake_probability:.4f}</span>
            {">=" if predicted_label == 1 else "<"} {threshold:.2f}
        </div>
        <div class="probability-card">
            <div style="display:flex;gap:0.5rem;flex-wrap:wrap;margin-bottom:0.65rem;">
                <span style="display:inline-block;padding:0.2rem 0.55rem;border-radius:999px;background:{true_color};color:#fff;font-size:0.9rem;">True</span>
                <span style="display:inline-block;padding:0.2rem 0.55rem;border-radius:999px;background:{fake_color};color:#fff;font-size:0.9rem;">Fake</span>
            </div>
            {probability_rows}
        </div>
        """,
